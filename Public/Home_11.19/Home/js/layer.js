   function showdiv1() {
 document.getElementById("bg1").style.display ="block";
 document.getElementById("show1").style.display ="block";
 }
function hidediv1() {
 document.getElementById("bg1").style.display ='none';
 document.getElementById("show1").style.display ='none';
 }

function showdiv2() {
 document.getElementById("bg2").style.display ="block";
 document.getElementById("show2").style.display ="block";
 }
function hidediv2() {
 document.getElementById("bg2").style.display ='none';
 document.getElementById("show2").style.display ='none';
 }

function showdiv3() {
 document.getElementById("bg3").style.display ="block";
 document.getElementById("show3").style.display ="block";
 }
function hidediv3() {
 document.getElementById("bg3").style.display ='none';
 document.getElementById("show3").style.display ='none';
 }
 
 function showdiv4() {
 document.getElementById("bg4").style.display ="block";
 document.getElementById("show4").style.display ="block";
 }
function hidediv4() {
 document.getElementById("bg4").style.display ='none';
 document.getElementById("show4").style.display ='none';
 }
 
function showdiv5() {
 document.getElementById("bg5").style.display ="block";
 document.getElementById("show5").style.display ="block";
 }
function hidediv5() {
 document.getElementById("bg5").style.display ='none';
 document.getElementById("show5").style.display ='none';
 }

function showdiv6() {
 document.getElementById("bg6").style.display ="block";
 document.getElementById("show6").style.display ="block";
 }
function hidediv6() {
 document.getElementById("bg6").style.display ='none';
 document.getElementById("show6").style.display ='none';
 }
 
 function showdiv8() {
 document.getElementById("bg8").style.display ="block";
 document.getElementById("show8").style.display ="block";
 }
function hidediv8() {
 document.getElementById("bg8").style.display ='none';
 document.getElementById("show8").style.display ='none';
}

 function showdiv9() {
 document.getElementById("bg9").style.display ="block";
 document.getElementById("show9").style.display ="block";
 }
function hidediv9() {
 document.getElementById("bg9").style.display ='none';
 document.getElementById("show9").style.display ='none';
}
 function showdiv10() {
 document.getElementById("bg10").style.display ="block";
 document.getElementById("show10").style.display ="block";
 }
function hidediv10() {
 document.getElementById("bg10").style.display ='none';
 document.getElementById("show10").style.display ='none';
}



